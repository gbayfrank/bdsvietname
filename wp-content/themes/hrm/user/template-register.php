		...
		<h3>
			...
			if (isset($_POST['task']) && $_POST['task'] == 'register') {
				if ($_SESSION['answer'] == $_POST['answer']) {
					...
					$phoneno = $wpdb->escape(trim($_POST['phoneno']));
					if ($email == "" || $pwd1 == "" || $pwd2 == "" || $username == "" || $phoneno == "") {
						$err = 'Vui lòng không bỏ trống những thông tin bắt buộc!';
					} else if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
						$err = 'Địa chỉ Email không hợp lệ!.';
					}  else if (!ctr_get_data_sql($phoneno) == "") {
						...
						if (is_wp_error($user_id)) {
							...
							// set the WP login cookieo
							$secure_cookie = is_ssl() ? true : false;
							wp_set_auth_cookie($user_id, true, $secure_cookie);
							update_usermeta(get_current_user_id(), 'phoneno', $phoneno); // cập nhật cho custom feild số chứng minh thư __trungctr-antopho
			?>
							<script>
			...
			<div class="mod-login clearfix">
				...
				<div class="mod-login-col2 clearfix">
					<div class="mod-input mod-login-input-email mod-input-email">
						...
						<input type="phoneno" name="phoneno" id="phoneno" placeholder="Số điện thoại" value="<?php echo ($_POST['phoneno']) ? trim($_POST['phoneno']) : '' ?>" required />
						<b></b>
						<span></span>
					</div>
					...
					<div class="mod-input mod-login-input-email mod-input-email">
						...
						<input type="email" name="email" id="email" placeholder="Email: example@abc.com" value="<?php echo ($_POST['email']) ? trim($_POST['email']) : '' ?>" required />
						<b></b>
						<span></span>
					</div>
								...
								<div class="mod-third-party-login">
									...
									<div class="mod-third-party-login-bd">
										<a href="<?php //echo home_url() 
													?>/wp-login.php?loginSocial=facebook" data-plugin="nsl" data-action="connect" data-redirect="current" data-provider="facebook" data-popupwidth="475" data-popupheight="175" class="btn-login-fb">
											<i class="fa fa-facebook" aria-hidden="true"></i> Facebook
										</a>
									</div>
									<div class="login-gg">
										<a href="<?php //echo home_url() 
													?>/wp-login.php?loginSocial=google" data-plugin="nsl" data-action="connect" data-redirect="current" data-provider="google" data-popupwidth="600" data-popupheight="600" class="btn-login-gg">
