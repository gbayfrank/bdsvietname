 ...
 * @since    1.0
 */
class HRM_Online_Support_Widget extends WP_Widget

{
...

	{
		...
		$this->WP_Widget( 'support-online', __( 'HRM - Online Support', 'fit' ), $widget_ops, $control_ops );
				...
				<div id="supporter-<?php echo $j + 1; ?>" class="<?php echo $j % 2 == 0 ? 'supp-odd' : 'supp-even'; ?> supporter">
					...
					<div class="supporter-online">
						<?php if ( !empty( $instance['supporter_skype'][$i][$j] ) ) : ?><p class="supporter-skype">
							<a href="<?php echo 'skype:' . $instance['supporter_skype'][$i][$j] . '?chat'; ?>"></a>
							</p><?php endif; ?>
						<?php if ( !empty( $instance['supporter_phone'][$i][$j] ) ) : ?>
							<p class="supporter-phone"><a href="tel:<?php echo $instance['supporter_phone'][$i][$j]; ?>"><?php echo $instance['supporter_phone'][$i][$j]; ?></a></p><?php endif; ?>
	...
	function form( $instance )
	{
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'supports_num_' . $i ); ?>"></br>
				<?php _e( 'Supporters', 'fit' ) ?></label>
		...
		<?php for ( $j = 0; $j < $supporter_num; $j++ ) : ?>
			<div style="width: 47%; padding: 5px; margin: 5px 0; background: #eee; <?php echo $j % 2 == 0 ? 'float: left;' : 'float: right;'; ?>">
				...
				<p>
					...
					<input type="text" id="<?php echo $this->get_field_id( 'supporter_phone_' . $i . $j ); ?>" name="<?php echo $this->get_field_name( 'supporter_phone' ); ?>[<?php echo $i; ?>][]" value="<?php echo esc_attr( $instance['supporter_phone'][$i][$j] ); ?>" size="25" />
				</p>

				<p>
					<label for="<?php echo $this->get_field_id( 'supporter_skype_' . $i . $j ); ?>"><?php _e( 'Skype', 'fit' ); ?></label>
					<input type="text" id="<?php echo $this->get_field_id( 'supporter_skype_' . $i . $j ); ?>" name="<?php echo $this->get_field_name( 'supporter_skype' ); ?>[<?php echo $i; ?>][]" value="<?php echo esc_attr( $instance['supporter_skype'][$i][$j] ); ?>" size="25" />
				</p>

				<p>
					<label for="<?php echo $this->get_field_id( 'supporter_yahoo_' . $i . $j ); ?>"><?php _e( 'Yahoo', 'fit' ); ?></label>
					<input type="text" id="<?php echo $this->get_field_id( 'supporter_yahoo_' . $i . $j ); ?>" name="<?php echo $this->get_field_name( 'supporter_yahoo' ); ?>[<?php echo $i; ?>][]" value="<?php echo esc_attr( $instance['supporter_yahoo'][$i][$j] ); ?>" size="25" />
